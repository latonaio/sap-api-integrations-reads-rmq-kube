...
import (
	...
	"io/ioutil"
	"net/http"
	sap_api_output_formatter "sap-api-integrations-xxxxxxxx-reads-rmq-kube/SAP_API_Output_Formatter"
	"strings"
	"sync"
...
func (c *SAPAPICaller) Header(xxxxxxxx string) {
	...
	err = c.outputter.Send(c.outputQueues[0], map[string]interface{}{"message": headerData, "function": "Header"})
	if err != nil {
		c.log.Error(err)
		return
	}
	...
	err = c.outputter.Send(c.outputQueues[0], map[string]interface{}{"message": itemData, "function": "ToItem"})
	if err != nil {
		c.log.Error(err)
		return
	}
...
func (c *SAPAPICaller) getQueryWithItem(req *http.Request, xxxxxxxx, xxxxxxxx string) {
	params := req.URL.Query()
	params.Add("$filter", fmt.Sprintf("XXXXXXXX eq '%s' and XXXXXXXX eq '%s' and XXXXXXXX eq '%s'", xxxxxxxx, xxxxxxxx))
	req.URL.RawQuery = params.Encode()
}
